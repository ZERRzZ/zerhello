...
模块文件一般用 `js` 后缀，但也有 `mjs` 后缀的，只是兼容不高。

## 导入映射

提供了一个相应的值，当模块 URL 被解析时将替换文本。
...
需要在 `<script type="module"></script>` 中使用 `import` `export` 或其他模块文件。
...
默认到导出：`export default xxx` 不加大括号，同样导入也不加 `import xxx from xxx`。

## 重命名导入导出

使用 `as` 关键字指定新名字。

也可统一创建模块对象：`import * as xxx from xxx`。

## 合并模块

在多个类似的模块的父级可以建一个 `index.ts`，在这个文件内导出所有子模块内容。

## 动态加载模块

允许你仅在需要时动态加载模块，将 `import()` 作为函数调用，将模块的路径作为参数传入。它返回一个 Promise，会兑现为一个可以让你访问其导出的模块对象。
