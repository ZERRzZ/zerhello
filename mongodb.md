...
- `db.集合名.insert(BSON格式数据)`：增加文档，会自动创建一个\_id 主键字段，有重复时报错
...
- 限制查询的个数：`db.集合名.find().limit(5)`：每次只查询 5 个
- 跳过多少条在查询：`db.集合名.find().skip(5)`：跳过 5 条再查询
...
- 利用`mongoose.connection.on`绑定事件，有 connected,error,disconnected 等
