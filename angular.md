...
属性型指令会更改元素, 组件或其他指令的外观或行为  
结构型指令会添加和删除 dom 元素来更改 dom 布局

**内置属性型指令**
----------------

`ngClass` 添加和删除一组 css 类, 可配合表达式和方法使用  
`ngStyle` 添加和删除一组 html 的内联样式  
`ngModel` 将数据双向绑定添加到 html 表单元素上
...
`*ngFor` 为列表中每个元素重复渲染一个节点, 一般语法 `let item of items`
