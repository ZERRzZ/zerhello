...
一般分为 `区块盒子` 和 `行内盒子`，盒子又有自己的 `内部显示` 和 `外部显示`

可以使用 `display` 属性来设置显示类型的各种值

### 外部显示

`block` 类型：产生换行；宽高属性生效；`padding` 和 `margin` 生效；宽度默认一般同容器一致

`inline` 类型：不产生换行；宽高属性不生效；`padding` 和 `margin` 但仅左右生效，上下不生效

`inline-block` 类型：不产生换行，但宽高属性生效，`padding` 和 `margin` 生效
